...
* **Sıralı Hızlı Skor Girişi (Sequential Numpad Flow)**: Bir takımın skorunu girdikten sonra klavye kapanmadan otomatik olarak diğer takımın girişine odaklanan, takım ismini dinamik olarak gösteren akıllı numpad akışı.
...
* **Responsive Mobil Uyum & Lockdown**: Web izleyici arayüzü mobil cihazlara (viewport kilitleri ve taşkın engellemeleriyle) %100 uyumludur; kullanıcı ekranı yanlışlıkla kaydıramaz veya büyütemez (Gerçek bir Native App hissi).
...
### 🛡️ 8. Veritabanı Hijyeni ve Firebase Güvenliği
* **Zombi Oda Temizliği**: Oyunlar sona erdiğinde (Maçı Bitir) Firebase üzerinde açık kalan tüm veriler (skorlar, mesajlar ve durum) `removeValue()` ile güvenli bir şekilde silinir. "Hayalet veriler" veritabanını şişirmez.
* **Otomatik TTL (Zaman Aşımı)**: Odalar ilk oluşturulduğunda `ServerValue.TIMESTAMP` ile işaretlenir. İlerleyen süreçte Cloud Functions ile 12 saati geçen eski odalar otomatik olarak temizlenebilir.
* **Kısmi Güncelleme (Partial Update)**: Uygulama skor güncellerken `updateChildren` mimarisi kullanarak veritabanını yormaz ve zaman damgası (timestamp) gibi diğer hassas verileri ezmeden çalışır.

### 📲 9. Akıllı Ekran Görüntüsü ve Özet Paylaşımı
...
### 🎨 10. Kod Tabanlı Animasyonlu Açılış Ekranı (AnimatedSplashScreen)
